...
These instructions were tested on 10.05.2025 with python 3.13.3 on linux, they worked.

1. Setup a venv `$ python -m venv .venv`
2. Activate the venv `$ source .venv/bin/activate`
3. Run `$ pip install -r requirements.txt`
4. Run `$ make clean reset` to clean & init database struct
5. Run `$ make tests` to make sure that everything is properly configurated
6. Copy some `.mp3` audio files into `mp3/` directory
7. Run `$ make fingerprint-songs` to analyze audio files & fill your db with hashes
8. Start play any of audio file (from any source) from `mp3/` directory, and run (parallely) `$ make recognize-listen seconds=5`
